...
public class SearchingVArray {
    ...
    private static int find(int[] array, int target, int low, int high){
        ...
        if(target == array[mid]) return array[mid];
            ...
            int found = (array[low] >= target)? find(array, target, low, mid - 1): -1;  //searching left
            if(found == -1) found = array[high] >= target? find(array, target, mid + 1, high): -1;  //no found, searching right
            return found;
        }
    }
